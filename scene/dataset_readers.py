...
def readColmapSceneInfo(path, images, args, llffhold=8):
    try:
    ...
    if args.eval:
        if os.path.basename(os.path.dirname(path)) == "nerfbuster":
            print("Nerfbuster train/val split has been activated")
            train_cam_infos = [c for c in cam_infos if "_1_" not in c.image_name]
            test_cam_infos = [c for c in cam_infos if "_1_" in c.image_name]
        else:
