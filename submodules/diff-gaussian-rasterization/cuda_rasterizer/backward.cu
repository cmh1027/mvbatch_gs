...
// each Gaussian.
__device__ void computeColorFromSH(int idx, int point_idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	const int* point_batch_index)
{
	...
	view += batch_idx * 16;
	proj += batch_idx * 16;
	campos = (glm::vec3*)((float3*)campos + batch_idx);
	radii += P * batch_idx;
	...
	if (shs)
		computeColorFromSH(idx, point_idx, D, M, (glm::vec3*)means, *campos, shs, clamped, (glm::vec3*)dL_dcolor, (glm::vec3*)dL_dmeans, (glm::vec3*)dL_dsh);
	...
	const int* __restrict__ point_batch_index)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= THREAD_SIZE)
	{
		...
		block.sync();
		const int progress = i * THREAD_SIZE + block.thread_rank();
		...
		for (int j = 0; !done && j < min(THREAD_SIZE, toDo); j++)
		{
			...
			const float c_beta = betas[collected_id[j]];
...
)
{
	...
	computeCov2DCUDA << <(BR + 255) / 256, 256 >> > (
		BR, P,
		means3D,
		radii,
		cov3Ds,
	...
	switch(B){
		case 1:
			renderCUDA<NUM_CHANNELS, 1> << <grid, block >> >(
				ranges, point_list, W, H, B, BR, bg_color, means2D, conic_opacity, colors, betas, depths, final_Ts, n_contrib, 
				dL_dpixels, dL_dpixels_beta, dL_dpixel_depths, dL_dmean2D, dL_dconic2D, dL_dopacity, dL_dcolor, dL_dbeta, dL_ddepths,
