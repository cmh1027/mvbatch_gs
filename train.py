...
from utils.loss_utils import pixel_loss, ssim, surface_ssim
...
def training(dataset, opt, pipe, args):
    ...
    if opt.gs_type == "3dgs":
        ...
        opt.noise_lr /= opt.batch_size 
  
    SH_interval = 1000

    if opt.iterations != 30000:
        div = 30000 / opt.iterations
        opt.opacity_reset_interval = opt.opacity_reset_interval // div
        opt.prune_interval = opt.prune_interval // div
        opt.densify_until_iter = opt.densify_until_iter // div
        opt.densify_from_iter = opt.densify_from_iter // div
        opt.densification_interval = opt.densification_interval // div
        opt.position_lr_max_steps = opt.position_lr_max_steps // div
        opt.log_batch_from = opt.log_batch_from // div
        opt.log_batch_interval = opt.log_batch_interval // div
        SH_interval = SH_interval // div
    ...
    initial_num_pts = gaussians.num_pts
    for iteration in range(first_iter, opt.iterations + 1): 
        gt_images = []
        xyz_lr = gaussians.update_learning_rate(iteration)

        # Every 1000 its we increase the levels of SH up to a maximum degree
        if iteration % SH_interval == 0:
        ...
        if opt.batch_size == 1:
            ...
            cam_idxs = [cam_idx]
            while len(cam_idxs) < opt.batch_size:
                if not viewpoint_stack:
                    viewpoint_stack = list(range(len(scene.getTrainCameras())))
                cam_idx = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))
                cam_idxs.append(cam_idx)
            cam_idxs = torch.tensor(cam_idxs, device=torch.device('cuda'))
        ...
        if opt.time_check:
            ...
            start = time.time()
        if opt.full_training_step > 0 and iteration > (opt.iterations - opt.full_training_step):
            _cams = cams
            for cam in _cams:
                cams = [cam]
                ...
                if opt.time_check:
                    torch.cuda.synchronize()
                    start = time.time()
                total_loss = total_loss/len(_cams)
                total_loss *= opt.loss_mult
                total_loss.backward()
                if opt.time_check:
                    torch.cuda.synchronize()
            ...
            depth = depth.detach() / (1-render_pkg["residual_trans"].detach() + 1e-6)
            ...
            if len(cams) > 1:
                ...
                if opt.lambda_dssim > 0:
                    ...
                    for i in range(len(cams)):
                        ...
                        if opt.ssim_sigma_end > 0:
                            ...
                            ssim_sigma_ = opt.ssim_sigma
                        ssim_sigma_ = ssim_sigma_ * depth[0].detach() / focal_mean
                        xyz_from_depth[collage_mask_binary[i] > 0] = xyz_from_depth_[collage_mask_binary[i] > 0]
                        ssim_sigma[collage_mask_binary[i] > 0] = ssim_sigma_[collage_mask_binary[i] > 0]

                    ssim_map = surface_ssim(image, collage_gt, xyz=xyz_from_depth, sigma=ssim_sigma, window_size=opt.ssim_window_size)
                ...
                loss = (1.0 - opt.lambda_dssim) * Ll
                if opt.lambda_dssim > 0:
                    xyz_from_depth = cams[0].depth_map_to_cam(depth[0].detach())
                    focal_mean = (cams[0].fx * cams[0].fy) ** 0.5
                    if opt.ssim_sigma_end > 0:
                        ssim_sigma = opt.ssim_sigma + (opt.ssim_sigma_end - opt.ssim_sigma) * (iteration / opt.iterations)
                    else:
                        ssim_sigma = opt.ssim_sigma
                    ssim_sigma = ssim_sigma * depth[0].detach() / focal_mean
                    ssim_map = surface_ssim(image, gt_image, xyz=xyz_from_depth, sigma=ssim_sigma, window_size=opt.ssim_window_size)
        ...
        with torch.no_grad():
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if opt.gs_type == "3dgs":
                        ...
                        if opt.predictable_growth:
                            ...
                            if opt.add_ratio > 100:
                                N_desification = (opt.add_ratio - opt.densify_from_iter) // opt.densification_interval
                                add_ratio = (opt.cap_max / initial_num_pts) ** (1 / N_desification) - 1
            ...
            if iteration < opt.iterations:
                ...
                if opt.gs_type == "mcmc":
                    ...
                    def op_sigmoid(x, k=100, x0=1-opt.prune_threshold):
